...
/** Allowed selection counts on step 3 by profile type. */
...
function isSelectionCountValidForProfile(profileType, count) {
  const { min, max } = getSelectionLimits(profileType);
...
function canRestoreDirectToStep5(payload) {
  ...
  if (
    ...
    !isSelectionCountValidForProfile(payload.profileType, payload.selectedItems.length)
  ) {
    return false;
  }
...
/**
 * Wire Show more / Show less on the meaning card (direct listeners — survives innerHTML because we re-bind after each render).
